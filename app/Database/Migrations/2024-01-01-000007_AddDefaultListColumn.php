...
class AddDefaultListColumn extends Migration
{
    public function up(): void
    {
        ...
        // Mark any existing list named "Pendientes" as default
        $this->db->query("UPDATE user_lists SET is_default = true WHERE name = 'Pendientes'");

        // Create "Pendientes" for users who still don't have a default list
        $this->db->query("
            INSERT INTO user_lists (user_id, name, description, is_public, is_default, created_at, updated_at)
            SELECT u.id, 'Pendientes', '', false, true, NOW(), NOW()
            FROM users u
            WHERE NOT EXISTS (
